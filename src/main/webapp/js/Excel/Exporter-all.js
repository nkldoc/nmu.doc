...
Ext.ux.Exporter.Button = Ext.extend(Ext.Button, {
  constructor: function(config) {
    ...
    if (config.store == undefined && config.component != undefined) {
      ...
      Ext.applyIf(config, {
          	component: { store: config.store },
      		buildShowHeadReport : 'sssssssss',
      });
    }
...
Ext.ux.Exporter.ExcelFormatter.Worksheet = Ext.extend(Object, {
  ...
  render: function(store) {
    return this.worksheetTpl.apply({
      ...
      rowCount : this.store.getCount() + (2 + this.buildShowHeadReport.length),// บวกแถวเพิ่ม
      title    : this.title,
      headShow : this.buildHeadShow().join("")
    });
  },
  ...
  buildHeadShow : function() { 
    var rows = [];
    Ext.each(this.buildShowHeadReport, function(value){
    	rows.push(String.format('<ss:Row ss:AutoFitHeight="1"><ss:Cell><ss:Data xmlns:html="http://www.w3.org/TR/REC-html40" ss:Type="String"><html:B><html:Font html:Size="12">{0}</html:Font></html:B></ss:Data><ss:NamedCell ss:Name="Print_Titles" /></ss:Cell></ss:Row>',value));
    });
    return rows;
  },
  buildRows: function() {
