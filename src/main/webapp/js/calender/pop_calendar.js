var	fixedX = -1			// x position (-1 if to appear below control)
	...
	var imgDir = "../js/calender/images/"			// directory for images ... e.g. var imgDir="/img/"

	var gotoString = "Go To Current Month"
	var todayString = "วันปัจจุบัน คือ"
	var weekString = "Wk"
    ...
    */
	...
	function closeCalendar() {
		var	sTmp
		hideCalendar();
		ctlToPlaceValue.value =	constructDate(dateSelected,monthSelected,parseInt(yearSelected+543))
	}
	...
	function selectYear(nYear) {
		yearSelected=parseInt(parseInt(nYear-543)+nStartingYear);
		yearConstructed=false;
		constructCalendar();
		popDownYear();
	}
	...
	function constructYear() {
		...
		if (!yearConstructed) {
			...
			j =	0
			nStartingYear =	parseInt(yearSelected+543)-3
			for	(i=(parseInt(yearSelected+543)-3); i<=(parseInt(yearSelected+543)+3); i++) {
				sName =	i;
				if (i==parseInt(yearSelected+543)){
					sName =	"<B>" +	sName +	"</B>"
				}
   ...
   function WeekNbr(n) {
	...
	function constructCalendar () {
		...
		document.getElementById("spanYear").innerHTML =	"&nbsp;" + parseInt(yearSelected+543)	+ "&nbsp;<IMG id='changeYear' SRC='"+imgDir+"drop1.gif' WIDTH='12' HEIGHT='10' BORDER=0>"
	}
			...
			if ( crossobj.visibility ==	"hidden" ) {
				...
				if ( formatChar	!= "" )
				{
					...
					for	(i=0;i<3;i++)
					{
						...
						{
							yearSelected = parseInt((aData[i]-543), 10)
							tokensChanged ++
						}
...
//				crossobj.top = fixedY==-1 ?	ctl.offsetTop +	toppos + ctl.offsetHeight +	2 :	fixedY
	...
	function checkDateFormat(obj, d_default) {
